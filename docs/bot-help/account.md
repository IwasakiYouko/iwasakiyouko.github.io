...
- `/自定义个人信息` `/cp`
  - 渲染游戏公开 profile 中的自定义个人信息页。指令后需要填写页序号，每次渲染一张，如 `/自定义个人信息 1`。
  - 仅支持查询自己的绑定账号，可以加 `u序号` 指定绑定列表中的账号，如 `/自定义个人信息 u2 3`。
...
- `/上传个人信息背景` `/上传个人信息图片` `/上传个人背景` `/上传个人信息` `/pjsk upload profile bg` `/pjsk upload profile background`
  - 上传自定义的个人信息背景图，需要消息内含有图片或回复上文中已有的某张图片。请注意，需要先进行游戏账号验证才能使用。
- `/清空个人信息背景` `/清除个人信息背景` `/清空个人信息图片` `/清除个人信息图片` `/pjsk clear profile bg` `/pjsk clear profile background`
  - 清除自定义的个人信息背景图。
- `/设置个人信息` `/设置个人信息背景` `/调整个人信息背景` `/调整个人信息` `/pjsk adjust profile` `/pjsk adjust profile bg` `/pjsk adjust profile background`
  - 调整自定义个人信息背景图，可选横屏/竖屏、模糊以及透明度。模糊度范围为 0~10，透明度范围为 0~100。如：`/调整个人信息 竖屏 模糊5 透明50`。

以上设置可加 `u序号` 操作自己的其他绑定账号，如 `/调整个人信息 u2 竖屏 模糊5 透明50`。
